...
# Copy both files first in order to use poetry to install all dependencies
...
# Making the "commands.sh" script executable and ensuring it has the right permissions
RUN chmod +x $APP_HOME/docker-scripts/commands.sh \
    && chown $USER_NAME:$USER_NAME $APP_HOME/docker-scripts/commands.sh \
    && chmod 755 $APP_HOME/docker-scripts/commands.sh
...
CMD ["/bin/sh", "/app/docker-scripts/commands.sh"]
